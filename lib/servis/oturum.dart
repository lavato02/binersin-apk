...
class Oturum {
  ...
  Future<bool> oturumac(BuildContext context, String mail, String sifre) async {
    ...
    http.Response sonuc = await http.post(
        Uri.parse(api_link + "?api_key=" + api_key), body: {
      'oturumac': 'true',
      'kul_mail': mail,
      'kul_sifre': sifre,
    });
      ...
      try {
        ...
        if (gelenJson != null && gelenJson is Map<String, dynamic>) {
          ...
          if (gelen["durum"] == "ok") {
            ...
            kaydetKullaniciEmail(gelen["bilgiler"]["kul_mail"]);
            kaydetbakiye(gelen["bilgiler"]["bakiye"]);
            if (gelen["bilgiler"] != null) {
              Kullanici kullanici = Kullanici.fromJson(gelen["bilgiler"]);
              // Şimdi "kul_mail" değerini başka bir değişkene atayabilir veya kullanabilirsiniz
            } else {
              print("Hata: bilgiler alanı null");
            }
  ...
  void kaydetKullaniciEmail(String? email) {
    if (email != null && email.isNotEmpty) {
      box.write("kullanici_email", email);
    }
  }

  String? getKullaniciEmail() {
    return box.read("kullanici_email");
  }
  void kaydetbakiye(String? bakiye) {
    if (bakiye != null && bakiye.isNotEmpty) {
      box.write("bakiye", bakiye);
    }
  }

  String? getbakiye() {
    return box.read<String>("bakiye");
  }
